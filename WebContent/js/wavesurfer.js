...
//For backwards compatibility
    ...
    WaveSurfer.util.extend(WaveSurfer.Drawer.Canvas, {
        ...
        drawWave: function (peaks, channelIndex, start, end) {
            ...
            // this.waveCc.fillStyle = (channelIndex == 0) ? this.params.waveColor : this.params.otherWaveColor;
            // if (this.progressCc) {
            //     this.progressCc.fillStyle = (channelIndex == 0) ? this.params.progressColor : this.params.otherProgressColor;
            // }
            this.waveCc.fillStyle = this.params.waveColor;
            if (this.progressCc) {
                this.progressCc.fillStyle = this.params.progressColor;
    ...
    WaveSurfer.util.extend(WaveSurfer.Drawer.MultiCanvas, {
        ...
        addCanvas: function () {
            ...
            // entry.otherWave = this.wrapper.appendChild(
            //     this.style(document.createElement('canvas'), {
            //         position: 'absolute',
            //         zIndex: 1,
            //         left: leftOffset + 'px',
            ...
            //     })
            // );
            // entry.otherWaveCtx = entry.otherWave.getContext('2d');
        ...
        drawLine: function (peaks, absmax, halfH, offsetY, start, end, channelIndex) {
            for (var index in this.canvases) {
                var entry = this.canvases[index];

                this.setFillStyles(entry);

                this.drawLineToContext(entry, entry.waveCtx, peaks, absmax, halfH, offsetY, start, end, channelIndex, false);
                this.drawLineToContext(entry, entry.progressCtx, peaks, absmax, halfH, offsetY, start, end, channelIndex, true);
            }
        },

        drawLineToContext: function (entry, ctx, peaks, absmax, halfH, offsetY, start, end, channelIndex, isProgress) {
            if (!ctx) { return; }
            ...
            if (channelIndex == 0) {
                ctx.fillStyle = isProgress ? this.params.progressColor : this.params.waveColor;
            } else if (channelIndex == 1) {
                ctx.fillStyle = isProgress ? this.params.otherProgressColor : this.params.otherWaveColor;
            }
